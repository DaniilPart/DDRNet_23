...
from core.criterion import CrossEntropy, OhemCrossEntropy, HybridLoss
...
def main():
    ...
    print(f"--> TensorBoard log directory is: {tb_log_dir}")
    ...
    elif config.LOSS.USE_HYBRID:
        print("Using Hybrid Loss (Dice + CrossEntropy)")
        criterion = HybridLoss(alpha=1.0, beta=0.8, ignore_label=config.TRAIN.IGNORE_LABEL)
    else:
